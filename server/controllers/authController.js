...
authController.directorSignup = async (req, res, next) => {
  ...
  const { first_name, last_name, email, password, phone_number, company_name } = req.body;
  ...
  bcrypt.hash(password, SALT_WORK_FACTOR)
    .then(async (hashedPassword) => {
      ...
      await pool.connect()
        .then((client) => {
          return client.query(queries.seedDirectorsTable, values)
            .then((response) => {
  ...
  await pool.connect()
    .then((client) => {
      client.query(queries.getDirector, values, (err, director) => {
        ...
        if (director.rows[0]) {
          bcrypt.compare(password, director.rows[0].password, (err, passwordMatch) => {
            client.release();
...
authController.actorSignup = async (req, res, next) => {
  ...
  const { headshot, resume, first_name, last_name, email, password, phone_number, city, facebook, twitter, instagram, youtube, bio } = req.body;
  ...
  bcrypt.hash(password, SALT_WORK_FACTOR)
    .then(async (hashedPassword) => {
      ...
      await pool.connect()
        .then((client) => {
          return client.query(queries.seedActorsTable, values)
            .then((response) => {
...
authController.validate = (method) => {
  switch (method) {
    case 'directorSignup': {
      return [
        body('first_name').isLength({ min: 1 }).withMessage('First Name is Required').trim().escape(),
        body('last_name').isLength({ min: 1 }).withMessage('Last Name is Required').trim().escape(),
    ...
    case 'actorSignup': {
      return [
        ...
        body('password').isLength({ min: 6 }).withMessage('Password should be minimum 6 characters'),
        body('phone_number').isMobilePhone().withMessage('Enter valid phone number'),
        body('city').isLength({ min: 1 }).withMessage('City is Required').trim().escape(),
        body('facebook'),
        body('twitter'),
        body('instagram'),
        body('youtube'),
        body('bio').isLength({ min: 1 }).withMessage('Bio is required').trim().escape(),
      ];
    }
    case 'actorLogin': {
      return [
