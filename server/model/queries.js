module.exports = {
  ...
  createDirectorWatchlistsTable: 'CREATE TABLE IF NOT EXISTS director_watchlists (director_id INTEGER NOT NULL, watchlist_id INTEGER NOT NULL, PRIMARY KEY (director_id, watchlist_id), CONSTRAINT director_watchlists_director_id_fkey FOREIGN KEY (director_id) REFERENCES directors (id)MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE, CONSTRAINT director_watchlists_list_id FOREIGN KEY (watchlist_id) REFERENCES watchlists (id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE CASCADE)',
